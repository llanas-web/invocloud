<script setup lang="ts">
    ...
    const { openModal: open, formState, pending, emailField, addEmail, onSubmit } = useSupplierUpdate();
...
<template>
    <UModal v-model:open="open" title="Modifier le fournisseur" description="Modifier le fournisseur">
        <template #body>
            <UForm :state="formState" class="space-y-4" @submit="onSubmit">
                ...
                <div class="w-full flex items-center gap-2">
                    <UInput v-model="emailField" class="flex-grow" placeholder="Ajouter un email" :disabled="pending" />
                    <UButton color="primary" variant="solid" icon="i-lucide-plus" @click="addEmail" />
                </div>
                <div class="flex justify-end gap-2">
                    <UButton label="Annuler" color="neutral" variant="subtle" @click="open = false"
                        :disabled="pending" />
                    <UButton label="Modifier" color="primary" variant="solid" type="submit" :loading="pending" />
                </div>
            </UForm>
        </template>
    </UModal>
