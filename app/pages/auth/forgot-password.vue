...
const { connectedUser, actions: { sendResetPasswordEmail } } = useAuth();
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
    await sendResetPasswordEmail.execute(event.data.email)
    ...
    <div class="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-32 min-h-full mb-12">
        <div class="flex-1 flex justify-end items-center mx-auto">
            <div class="flex flex-col items-center justify-center gap-4 p-4 max-w-lg">
                ...
                }" :disabled="sendResetPasswordEmail.pending.value" @submit="onSubmit" :submit="{
                    label: 'Envoyer le lien de réinitialisation',
                    loading: sendResetPasswordEmail.pending.value,
                    color: 'primary',
                    variant: 'solid',
                    disabled: emailSent
                }">
